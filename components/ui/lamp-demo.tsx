...
/* Drop common motion props so they never reach a plain DOM node */
...
export const LampContainer: React.FC<LampProps> = ({
  ...
  return (
    <div
      ...
      data-theme={tone}
      className={cn(
        "relative flex min-h-screen flex-col items-center justify-center overflow-hidden w-full rounded-md",
        className
      )}
      style={{
        backgroundColor: "hsl(var(--background))",
        color: "hsl(var(--foreground))",
        // default beam angles come from CSS vars
      }}
    >
      <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0">
        <MotionDiv
          ...
          animate={useMotion ? { opacity: 1, width: "30rem" } : undefined}
        ...
        <div aria-hidden="true" className="absolute top-1/2 h-48 w-full translate-y-12 scale-x-150 bg-black blur-2xl pointer-events-none" />
        <div aria-hidden="true" className="absolute top-1/2 z-50 h-48 w-full bg-transparent opacity-10 backdrop-blur-md pointer-events-none" />

        <MotionDiv
          aria-hidden="true"
          animate={useMotion ? { scale: [1, 1.06, 1] } : undefined}
          transition={useMotion ? { duration: 6, repeat: Infinity, ease: "easeInOut" } : undefined}
        ...
        <MotionDiv
          ...
          animate={useMotion ? { width: "16rem" } : undefined}
          transition={useMotion ? { delay: 0.25, duration: 0.85, ease: "easeInOut" } : undefined}
        ...
        <MotionDiv
          ...
          animate={useMotion ? { width: "30rem" } : undefined}
          transition={useMotion ? { delay: 0.25, duration: 0.85, ease: "easeInOut" } : undefined}
          className="absolute inset-auto z-50 h-0.5 w-[30rem] -translate-y-[7rem] pointer-events-none"
          style={{ backgroundColor: beamColors[1] }}
        />

        {/* subtle noise overlay inline SVG */}
        <svg className="absolute inset-0 z-60 pointer-events-none opacity-5" width="100%" height="100%" preserveAspectRatio="none" aria-hidden="true">
